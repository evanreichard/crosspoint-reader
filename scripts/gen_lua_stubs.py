...
annotations above each binding in src/util/lua/LuaBindings*.cpp.
...
SOURCES = sorted((REPO_ROOT / "src" / "util" / "lua").glob("LuaBindings*.cpp"))
...
def parse_annotations(text: str) -> tuple[dict[str, Binding], dict[str, list[str]]]:
    ...
    for line in text.splitlines():
        ...
        if stripped.startswith("---"):
            ...
            continue
        # clang-format reflows doc comments at 120 columns, so a long description arrives as a bare
        # `// ...` continuation. Without this it would flush the block and silently drop the binding.
        if is_comment and description and not params and not returns and not stripped.startswith("-- @"):
...
def parse_registration(text: str) -> tuple[dict[str, str], dict[str, str], dict[str, list[tuple[str, str]]]]:
    """Returns ({c_name: lua_name}, {global: kind}, {table: [(name, literal)]}) from the register*() bodies."""
    bodies = re.findall(r"void luabindings::register\w+\(lua_State\* state\) \{(.*?)\n\}", text, re.DOTALL)
    if not bodies:
        raise ValueError("no luabindings::register*() definitions found")
    body = "\n".join(bodies)
